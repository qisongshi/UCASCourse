...
int main() {
    ...
    int32_t *predict_matrix = (int32_t *)malloc(rows / 2 * cols / 2 * sizeof(int32_t));
    ...
    file = fopen("result_matrix.bin", "rb");
    if (file != NULL) {
        fread(result_matrix, sizeof(int32_t), rows / 2 * cols / 2, file);
        fclose(file);
        printf("Matrix has been read from result_matrix.bin\n");
    ...
    for(int i = 0; i < cf; i++)
    {
        for(int j = 0; j < (rows / cf / 2); j++)
        {
            for(int k = 0; k < (cols / 2); k++)
                predict_matrix[(j + i * rows / cf / 2) * cols / 2 + k] = tmp_matrix[(j + i * rows / cf) * cols / 2 + k] + tmp_matrix[(j + i * rows / cf + rows / cf / 2) * cols / 2 + k];
        }
    }
    ...
    for(int i = 0; i < rows / 2; i++)
    {
        ...
        {
            if(predict_matrix[i * cols / 2 + j] != result_matrix[i * cols / 2 + j])
            {
                printf("output false!\n");
                break;
            }
    ...
    // {
    //     for(int j = 0; j < cols / 2; j++)
    //         printf("%d ", tmp_matrix[i * cols / 2 + j]);
    //     printf("\n");
    // }
    // for(int i = 0; i < rows / 2; i++)
    // {
    //     for(int j = 0; j < cols / 2; j++)
    //         printf("%d ", predict_matrix[i * cols / 2 + j]);
    //     printf("\n");
    // }
    // 释放内存
    free(matrix);
